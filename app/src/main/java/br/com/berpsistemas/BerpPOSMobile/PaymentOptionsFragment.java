...
public class PaymentOptionsFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        // Atualiza o TextView com os detalhes do pagamento
        TextView tvPaymentDetails = view.findViewById(R.id.tv_payment_details);
        tvPaymentDetails.setText(new StringBuilder()
                .append("Atendimento ")
                .append(paymentId)
                .append(" - Valor: ")
                .append(NumberFormat.getCurrencyInstance(new Locale("pt", "BR")).format(paymentValue))
                .toString());

        // Configuração dos CARDS de pagamento (ao invés de botões)
        CardView cardDinheiro = view.findViewById(R.id.btn_dinheiro);
        CardView cardCredito = view.findViewById(R.id.btn_credito);
        CardView cardDebito = view.findViewById(R.id.btn_debito);
        CardView cardPix = view.findViewById(R.id.btn_pix);
        Button btnCancel = view.findViewById(R.id.btn_cancel);

        // Listeners dos cards
        cardDinheiro.setOnClickListener(v -> {
            if (listener != null) {
                listener.onPaymentMethodSelected("dinheiro");
            }
        });

        cardCredito.setOnClickListener(v -> {
            if (listener != null) {
