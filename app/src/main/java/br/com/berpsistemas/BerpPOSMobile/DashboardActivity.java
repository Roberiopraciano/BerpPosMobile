...
public class DashboardActivity extends AppCompatActivity implements View.OnClickListener,OpcoesDialogFragment.OnOptionSelectedListener {
    ...
    private CardView itens,mesas,conta,pagamento,cardTransactionHistory;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cardTransactionHistory.setOnClickListener(this);
...
// Opcional: Alterar a cor para indicar que está desabilitado
        ...
        Configuracao mesaConfig = Variaveis.getConfiguracao("MOBILE_VENDA_MESA","S");
        Configuracao cartaoConfig = Variaveis.getConfiguracao("MOBILE_VENDA_CARTAO","N");
        Configuracao balcaoConfig = Variaveis.getConfiguracao("MOBILE_VENDA_BALCAO","N");
        Configuracao deliveryConfig = Variaveis.getConfiguracao("MOBILE_VENDA_DELIVERY","N");
    ...
    private void realizarLogout() {
        ...
        if (Variaveis.getToken() != null) {
            Proxy.invalidarSessao(Variaveis.getToken()).thenAccept(response -> {
                ...
                runOnUiThread(() -> {
                    FancyToast.makeText(this, "Erro ao fazer logout: " + e.getMessage(),FancyToast.LENGTH_LONG,FancyToast.ERROR,true).show();
                });
                return null;
            });
        }
    ...
    private CompletableFuture<Boolean> validarSessao() {
        return Proxy.validarSessao(Variaveis.getToken())
                .thenApply(response -> {
                    ...
                    runOnUiThread(() -> {
                        FancyToast.makeText(this, "Erro ao validar a sessão: " + e.getMessage(), FancyToast.LENGTH_LONG,FancyToast.WARNING,true).show();
                    });
        ...
        if (view == itens  ) {
            ...
            validarSessao().thenAccept(isValid -> {
                if (isValid) {
                    ...
                    runOnUiThread(() -> {
                        FancyToast.makeText(this, "Sessão inválida. Redirecionando para login.",FancyToast.LENGTH_LONG,FancyToast.WARNING,true).show();;
                    });
            ...
            validarSessao().thenAccept(isValid -> {
                if (isValid) {
                    ...
                    // Sessão inválida, redireciona para a tela de login
                    runOnUiThread(() -> {
                        FancyToast.makeText(this, "Sessão inválida. Redirecionando para login.",FancyToast.LENGTH_LONG,FancyToast.ERROR,true).show();
                    });
                    Variaveis.setToken(""); // Limpa o token localmente
                    Intent i = new Intent(this, LoginActivity.class);
                    startActivity(i);
            ...
            AlertDialog dialog = builder.create();
            dialog.show();
        } else if (view == cardTransactionHistory) {
            validarSessao().thenAccept(isValid -> {
                if (isValid) {
    ...
    private class Tarefa extends AsyncTask<String, String, Integer> {
        ...
        protected void onPostExecute(Integer result) {
            if(result == 1){
                FancyToast.makeText(context, "Carga Efetuada com Sucesso", FancyToast.LENGTH_LONG,FancyToast.INFO,true).show();
            }else{
                FancyToast.makeText(context,exception,FancyToast.LENGTH_LONG,FancyToast.WARNING,true).show();
            }
            dialog.dismiss();
        }
