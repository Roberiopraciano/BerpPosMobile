...
//				edtQtd.setText("");
    ...
    private static class ParsedItem {
        String codigo;
        String descricao;
        Double valor; // null se não der pra parsear
    }


    /**
     * Faz o parse de "xxx-ddddd-vvvv" usando primeiro e último '-' para
     * tolerar descrições com hífen no meio.
     */
    private ParsedItem parseListItem(String raw) {
        ...
        if (first >= 0 && last > first) {
            p.codigo    = raw.substring(0, first).trim();
            p.descricao = raw.substring(first + 1, last).trim();
            String vStr = raw.substring(last + 1).trim();
            p.valor     = parseMoneyToDouble(vStr);
        } else {
            // formato inesperado: joga tudo em descrição, deixa código/valor em branco
            p.codigo = "";
            p.descricao = raw.trim();
            p.valor = null;
        }
        return p;
    }
    ...
    /**
     * Converte string de dinheiro pt-BR ("1.234,56") ou EN ("1234.56") para double.
     * Remove símbolos e tenta detectar se há vírgula decimal.
     */
    private Double parseMoneyToDouble(String s) {
        if (s == null) return null;
        String only = s.replaceAll("[^0-9,\\.\\-]", "").trim();
        if (only.isEmpty()) return null;

        try {
            // Se tem vírgula e não tem ponto: assume formato BR "1.234,56"
            if (only.contains(",") && !only.contains(".")) {
                only = only.replace(".", "").replace(",", "."); // tira milhares e troca decimal
            } else if (only.contains(",") && only.contains(".")) {
                // Heurística: se a última vírgula vem depois do último ponto, vírgula é decimal
                int li = only.lastIndexOf(',');
                int lp = only.lastIndexOf('.');
                if (li > lp) {
                    only = only.replace(".", "").replace(",", ".");
                } else {
                    // vírgula de milhares, ponto decimal -> remove vírgulas
                    only = only.replace(",", "");
                }
            } else {
                // Só dígitos e possivelmente ponto decimal EN
                // nada a fazer
            }
    ...
    public AdapterView.OnItemClickListener getItemSelecionado(){
        ...
        return (arg0, view, position, index) -> {
            ...
            String raw = proStrings.get(position);
            ParsedItem p = parseListItem(raw);

            // Campo 1
            if (edtxtdescprod.hasFocus()) {
                edtxtdescprod.setText(p.descricao);
                edtxtcodprod.setText(p.codigo);
                listView1.setVisibility(View.GONE);

                if (LAYOUT == 0) {
                    Double valor = p.valor;
                    if (valor == null) {
                        // fallback para o modelo (mantém sua lógica atual)
                        try {
                            String valStr = BerpModel.prodValor(p.codigo);
                            valor = parseMoneyToDouble(valStr);
                        } catch (Exception ignored) {}
                    }
                    if (valor != null) {
                        txtValorProdT.setText(
                                NumberFormat.getCurrencyInstance(myLocale).format(valor)
                        );
                    } else {
                        txtValorProdT.setText(""); // ou manter como estiver
                    }
                }
            }
            ...
            // Campo 2
            if (edtxtdescprod2.hasFocus()) {
                edtxtdescprod2.setText(p.descricao);
                edtxtcodprod2.setText(p.codigo);
                listView2.setVisibility(View.GONE);
            }

            // Campo 3
            if (edtxtdescprod3.hasFocus()) {
                edtxtdescprod3.setText(p.descricao);
                edtxtcodprod3.setText(p.codigo);
                listView3.setVisibility(View.GONE);
            }

            // Campo 4
            if (edtxtdescprod4.hasFocus()) {
                edtxtdescprod4.setText(p.descricao);
                edtxtcodprod4.setText(p.codigo);
                listView4.setVisibility(View.GONE);
            }
        };
    ...
    public void onClick(View view) {
        ...
        if(view == btnVoltar){
            ...
            if(vlrdividido > 0){
                ...
                if (Variaveis.getConfiguracao("CONTROLAR_QTD_MAXIMA_POR_ITEN_VENDA","N").getValor().equalsIgnoreCase("S")) {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK)
        {
            if (!BerpModel.isMesaEmpty(BerpModel.getNumMesa())) {
                ...
                new AlertDialog.Builder(this).setTitle("AVISO!")
                        ...
                        .setPositiveButton("Sim", (dialog, which) -> ItensMesaActivity.this.finish())
                        .setNegativeButton("Cancelar", null)
                        .setCancelable(false).show();

                return true; // <- AQUI É O PROBLEMA! Isso deve estar aqui dentro
            } else {
                ItensMesaActivity.this.finish();
            }
        }
