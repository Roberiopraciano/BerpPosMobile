...
import br.com.berpsistemas.BerpPOSMobile.ContaActivity;
import br.com.berpsistemas.BerpPOSMobile.ItensMesaActivity;
...
public class RecyclerAdapterMesas extends RecyclerView.Adapter<ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int position) {
        ...
        viewHolder.itemView.setOnClickListener(view1 -> {
            ...
            String pergunta="Lançar itens";

            if (mesa.getStatus().equals("3")|| mesa.getStatus().equals("70")){
              pergunta = "Reabrir e Lancar Itens";
            }


            builder.setPositiveButton(pergunta, (dialogInterface, i1) -> {
                if (mesa.getStatus().equals("3") || mesa.getStatus().equals("70")) {
                    try {
                        // Show a loading message
                        FancyToast.makeText(contextInternal, "Reabrindo atendimento..." ,FancyToast.LENGTH_LONG,FancyToast.INFO,true).show();

                        // Call the async method with proper callbacks
                        BerpModel.reAbrirMesaAsync(
                                mesa.getId(),
                                success -> ((Activity) contextInternal).runOnUiThread(() -> {
                                    if (success) {
                                       FancyToast.makeText(contextInternal,"Mesa reaberta com sucesso!",FancyToast.LENGTH_LONG,FancyToast.SUCCESS,true).show();;

                                        startItensMesaActivity(mesa);
                                    } else {
                                        FancyToast.makeText(contextInternal,
                                                "Não foi possível reabrir o Atendimento.",FancyToast.LENGTH_LONG,FancyToast.ERROR,true).show();;

                                    }
                                }),
                                error -> ((Activity) contextInternal).runOnUiThread(() -> {
                                    FancyToast.makeText(contextInternal,
                                            "Erro ao reabrir: " + error.getMessage()
                                            ,FancyToast.LENGTH_LONG,FancyToast.ERROR,true).show();;
                                })
                        );
                    } catch (Exception e) {
                        FancyToast.makeText(contextInternal,
                                "Erro ao reabrir atendimento: " + e.getMessage()
                                ,FancyToast.LENGTH_LONG,FancyToast.ERROR,true).show();;
                    }
                } else {
                    // If the table is not awaiting payment, just launch ItensMesaActivity
                    startItensMesaActivity(mesa);
                }
            });
    ...
    private void startItensMesaActivity(Mesa mesa) {
        ...
        BerpModel.setTpvend(mesa.getTipoVenda());
