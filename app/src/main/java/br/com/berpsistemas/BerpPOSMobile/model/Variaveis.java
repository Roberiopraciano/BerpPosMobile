...
public class Variaveis {
    ...
    public static Configuracao getConfiguracao(String nome, String valorPadrao) {
        ...
        for (Configuracao cfg : configuracoes) {
            if (cfg.getNome().equalsIgnoreCase(nome)) {
                return cfg;
            }
        }

        // se chegou aqui, não encontrou: cria com valor padrão
        Configuracao nova = new Configuracao(nome, valorPadrao);
        configuracoes.add(nova);
        return nova;
    }
                            ...
                            if (millisAtual >= millisInicio && millisAtual <= millisFim) {
                                ...
                                if (!dataAtual.before(dataInicio) && !dataAtual.after(dataValidade)) {
                                    if (tipo == 1 || (tipo == 0 && getProdutoComb(produto.getCod()) != null)) {
                                        autoComplete.add(produto.getCod()+"-"+produto.getDesc() +"-"+produto.getValorUnitarioFormatado());
                                    }
                                }
                            }
                        }
