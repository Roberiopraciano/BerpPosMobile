...
import br.com.berpsistemas.BerpPOSMobile.model.PedidoItemAdapter;
...
public class ContaActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private TextView txtSubTotalConta,txtServiConta,txtDesccConta,txtValorConta,txtvlrContaDiv,txtvlrPagamentos;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtvlrPagamentos = findViewById(R.id.txtPagamentosRecebidos);
        ...
        new ConnServer(this, LAYOUT).execute();
    ...
    public void onClick(View view) {
        ...
        if (btntipoconta == view) {
            LAYOUT = (LAYOUT == 1 ? 0 : 1);
            btntipoconta.setText(LAYOUT == 1
                    ? R.string.label_detalhado
                    : R.string.label_simples);

            // dispara a task que vai atualizar mesa e, no onPostExecute, reconfigura o adapter
            new ConnServer(this,LAYOUT).execute();
...
//    }
    ...
    private class ConnServer extends AsyncTask<Integer, String, Integer> {
        private Context context;
        private int tipo;

        ConnServer(Context context, int tipo) {
        ...
        protected void onPostExecute(Integer result) {
            ...
            dialog.dismiss();
            if (result != 0) {
                FancyToast.makeText(context, exceptMsg,FancyToast.LENGTH_LONG,FancyToast.WARNING,false).show();
                finish();
                return;
            }

            // … aqui você já tem mesa atualizada e LAYOUT definido …
            pedidoAdapter = new PedidoItemAdapter(
                    context,
                    R.layout.list_itemconta,
                    mesa.getProdutosAsVector(),
                    LAYOUT
            );
            list.setAdapter(pedidoAdapter);

            atualizarEstadoUI();  // mostra/oculta labels, etc.

            // TODO Auto-generated catch block
            if (result != 0) {
                FancyToast.makeText(context, exceptMsg, FancyToast.LENGTH_LONG,FancyToast.ERROR,true).show();
                finish();
               ...
               // startActivity(i);
                ...
                txtvlrContaDiv.setText(mesa.getVlrBruto());
                txtvlrPagamentos.setText(mesa.getTotalPagoStr());



                String contadividida = mesa.getVlrBruto();

                if (contadividida != null && !contadividida.isEmpty()) {
                    contadividida = contadividida.replace(".", "").trim();
                    ...
                    try {
                        ...
                        FancyToast.makeText(context, "Erro ao processar valor da conta: " + contadividida, FancyToast.LENGTH_LONG,FancyToast.INFO,true).show();
                    }
                } else {
                    vlrdividido = 0.0; // Valor padrão para strings vazias ou nulas
                    FancyToast.makeText(context, "Valor da conta não disponível.", FancyToast.LENGTH_LONG,FancyToast.INFO,true).show();
                }
        ...
        protected Integer doInBackground(Integer... params) {
            ...
            try {
                ...
                mesa = Proxy.visualizaConta(BerpModel.getId(), tipo).get();
