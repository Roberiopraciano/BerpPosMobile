...
public class PaymentCallbackHandler implements IPaymentCallbackHandler {
    ...
    public void handleCallback(Context context, Intent intent) {
        try {
            ...
            FancyToast.makeText(context, "Erro no callback: " + e.getMessage(),FancyToast.LENGTH_LONG,FancyToast.ERROR,true).show();
    ...
    private void handlePaymentCallback(Context context, JSONObject jsonResponse) throws JSONException {
        ...
        String doc = payment.optString("cieloCode", "");
        String terminal = payment.optString("terminal", "");
        String idOrder = jsonResponse.optString("id","");
        String idAtendimento =paymentFields.optString("paymentTransactionId","");
        double amount = payment.optDouble("amount", 0.0) / 100.0;

        Log.d(TAG, "Pagamento recebido: brand=" + brand + ", authCode=" + authCode + ", statusCode=" + statusCode);

        switch (statusCode) {
            case "0":
            case "1":
                FancyToast.makeText(context, "Pagamento aprovado!", FancyToast.LENGTH_LONG,FancyToast.INFO,true).show();

                break;

            case "2":
                FancyToast.makeText(context, "Pagamento cancelado", FancyToast.LENGTH_LONG,FancyToast.INFO,true).show();
                if (paymentListener != null) {
                    paymentListener.onPaymentCancelled();
                } else {
                    Log.e(TAG, "Listener não registrado para processar cancelamento");
                }
                break;
    ...
    private void notifyError(Context context, String reason) {
        Log.e(TAG, reason);
        FancyToast.makeText(context, reason, FancyToast.LENGTH_LONG,FancyToast.INFO,true).show();
        if (paymentListener != null) {
